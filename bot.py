...
# Визначте змінну requires_plan як булеву (логічну) змінну
requires_plan = True
...
def some_function(update, context, chat_id):
    ...
    some_function_new(update, context) # removed chat_id argument from the recursive call
...
def some_function_new(update, context):
    chat_id = update.message.chat_id
...
def save_order_to_database(update, context):
    ...
    context.bot.send_message(chat_id=os.environ['HEROKU_APP_NAME'])
...
bot_message = f"Дякую за оплату, ваша робота в процесі виконання. Надішлю готову роботу так скоро як зможу але не пізніше ніж вказана ваша дата."
